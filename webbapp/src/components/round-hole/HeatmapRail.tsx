...
export function HeatmapRail({
  ...
  return (
    <>
      <button
        onClick={onToggle}
        aria-label={isOpen ? 'Dölj caddy heatmap' : 'Visa caddy heatmap'}
        aria-pressed={isOpen}
        className={`absolute right-0 top-1/2 -translate-y-1/2 z-10 w-8 h-14 rounded-l-lg shadow-lg flex items-center justify-center text-lg font-black border-l border-y border-slate-300 ${
          isOpen ? 'bg-green-600 text-white border-green-700' : 'bg-white/95 text-ink'
        }`}
      >
        ▦
      </button>
      ...
      {isOpen ? (
        <div className="absolute right-9 top-1/2 -translate-y-1/2 z-10 flex flex-col gap-1 max-h-[70vh] overflow-y-auto py-2 px-1 bg-slate-900/70 rounded-l-lg">
