...
async function seedRound(opts: {
  ...
  strokes: number[]; // per hål, samma längd som course.holes
  ...
  // Idempotent: hitta befintlig runda för (user, course, startedAt-dag).
  ...
  const finishedAt = new Date(startedAt.getTime() + 3.5 * 3600 * 1000); // ~3.5h
...
async function follow(followerId: string, followingId: string) {
  await prisma.userFollow.upsert({
    ...
    create: { followerUserId: followerId, followingUserId: followingId },
    update: {}
  });
}
...
async function main() {
  ...
  log('Skapar bana med 9 hål (Parkbanan)...');
  ...
  log('Skapar 18-hålsbana (Skogsbanan)...');
  ...
  log("Lägger till caddy-slag för flera spelare...");

  // Anna — full uppsättning
  ...
  // Lisa — nybörjardata med större spridning
  ...
  log('Lägger till mission-historik...');
  ...
  log('Skapar spelade rundor (avslutade)...');

  // Anna — 3 rundor på Parkbanan, förbättring över tid
  await seedRound({
    user: anna,
    course: parkbanan,
    startedDaysAgo: 28,
    strokes: [5, 4, 7, 5, 5, 4, 7, 5, 6], // 48
    status: 'COMPLETED'
  });
  await seedRound({
    user: anna,
  ...
  await seedRound({
    ...
    course: parkbanan,
    startedDaysAgo: 3,
    strokes: [4, 4, 6, 4, 4, 3, 6, 4, 5], // 40 — par är 36, så +4
    status: 'COMPLETED'
  });

  // Erik — 2 rundor: en på Skogsbanan (18 hål), en på Parkbanan
  await seedRound({
    user: erik,
    course: skogsbanan,
    startedDaysAgo: 21,
    strokes: [4, 5, 5, 4, 3, 5, 4, 4, 6, 4, 4, 5, 4, 4, 3, 5, 5, 4], // 78
    status: 'COMPLETED'
  });
  await seedRound({
    user: erik,
    course: parkbanan,
    startedDaysAgo: 5,
    strokes: [4, 3, 5, 4, 4, 3, 5, 4, 4], // 36 — par
    status: 'COMPLETED'
  });

  // Lisa — 2 rundor, nybörjarscores
  await seedRound({
    user: lisa,
    course: parkbanan,
    startedDaysAgo: 18,
    strokes: [7, 5, 8, 6, 6, 5, 8, 7, 7], // 59
    status: 'COMPLETED'
  });
  await seedRound({
    user: lisa,
    course: parkbanan,
    startedDaysAgo: 6,
    strokes: [6, 4, 7, 6, 6, 4, 7, 6, 6], // 52
    status: 'COMPLETED'
  });

  // Markus — 2 rundor, scratch-nivå
  await seedRound({
    user: markus,
    course: skogsbanan,
    startedDaysAgo: 17,
    strokes: [4, 4, 5, 4, 3, 4, 4, 4, 5, 4, 4, 5, 4, 4, 3, 4, 5, 4], // 74
    status: 'COMPLETED'
  });
  await seedRound({
    user: markus,
    course: parkbanan,
    startedDaysAgo: 2,
    strokes: [3, 3, 4, 4, 3, 3, 5, 4, 4], // 33 — -3 mot par
    status: 'COMPLETED'
  });

  log('Sätter upp följ-relationer...');
  // Alla följer Markus (han är "stjärnan")
  ...
  // Lisa följer Anna för tips
  await follow(lisa.id, anna.id);
  // Anna följer admin för missions
